...
**electionlens** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `electionlens` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
electionlens ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
electionlens ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
electionlens ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `electionlens` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="electionlens") for rec in records]   # records = your JSON output
