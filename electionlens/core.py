...
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'EL-CLUSTER-001', 'severity': 'high', 'weight': 2.5, 'title': 'COORD_CLUSTER', 'when': {'cluster_size': '^([5-9]|\\d{2,})$', 'sync_post_pct': '^([7-9]\\d|100)'}, 'remediation': "Refer to platform's inauthentic-behavior team."}, {'id': 'EL-NARR-001', 'severity': 'medium', 'weight': 2.0, 'title': 'NARRATIVE_INJECTION', 'when': {'narrative_injection': True}, 'remediation': 'Document; do not amplify; consider counter-comms.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="election-watch",
                    metadata=item,
                ))
