"""Native cognis-connect emit for electionlens — forward findings to any platform.

Maps electionlens's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    electionlens ... --format json | electionlens-emit --to stix
    electionlens-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out['title'] = rec.get('pattern', 'unknown')
        out['severity'] = rec.get('risk_level', 'info')
        ...
        out['url'] = rec.get('url')
        out['sha256'] = rec.get('hash')
        out['cve'] = rec.get('cve')
        out['imo'] = rec.get('imo')
        out['mmsi'] = rec.get('mmsi')
        out['lat'] = rec.get('latitude')
        out['lon'] = rec.get('longitude')
        return out
    except Exception:
        return rec
